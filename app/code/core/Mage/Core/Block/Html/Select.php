...
class Mage_Core_Block_Html_Select extends Mage_Core_Block_Abstract
{
    ...
    protected function _optionToHtml($option, $selected = false)
    {
		...
		$allTxt = Mage::helper('catalogsearch')->__('All');
		
		$label = $this->escapeHtml($option['label']);
		$labelWhiteRemove = preg_replace("/[\s_]/", "-", $label);
		$pLabelFinal = strtolower($labelWhiteRemove );
		
			if($label != $allTxt) {
			$dataIMG =  'data-image="'.Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).'wysiwyg/'.$pLabelFinal.'.png"';
			}
	
        return sprintf('<option '.$dataIMG.' value="%s"%s %s>%s</option>',
